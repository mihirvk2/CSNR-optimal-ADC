...
def msqe_gaussian(N, p, t1, tM, b_adc, delta_imc):
    ...
    error_right, _ = quad(lambda x: (x - rep_high)**2 * norm.pdf(x, loc=mu, scale=sigma),
                           tM, np.inf)
    msqe += error_left + error_right
    
    return msqe, True

def msqe_thresholds_sweep_gaussian(N, p, b_adc, delta_imc, npoints):
    ...
    logging.basicConfig(filename=f'Outputs/msqe_thresholds_sweep_N_{N}_b_adc_{b_adc}_delta_imc_{delta_imc}.log',
    ...
    for i in tqdm(range(npoints), desc="Sweeping t1"):
        ...
        for j in tqdm(range(i+1, npoints), desc="Sweeping tM", leave=False):
            ...
            msqe_val_cur, valid = msqe_gaussian(N, p, t1_cur, tM_cur, b_adc, delta_imc)
            if valid:
                t1_list.append(t1_cur)
                tM_list.append(tM_cur)
                msqe_val_list.append(msqe_val_cur)
                logging.info(f"For t1 = {t1_cur} and tM = {tM_cur}, msqe = {msqe_val_cur}")
    
    t1_array = np.array(t1_list)
    tM_array = np.array(tM_list)
    msqe_array = np.array(msqe_val_list)
    
    # Save the data
    np.save(f'Outputs/msqe_thresholds_sweep_t1_N_{N}_b_adc_{b_adc}_delta_imc_{delta_imc}.npy', t1_array)
    np.save(f'Outputs/msqe_thresholds_sweep_tM_N_{N}_b_adc_{b_adc}_delta_imc_{delta_imc}.npy', tM_array)
    np.save(f'Outputs/msqe_thresholds_msqe_val_N_{N}_b_adc_{b_adc}_delta_imc_{delta_imc}.npy', msqe_array)
    

def plot_msqe(N, b_adc, delta_imc):
    # Visualizes the MSE_q sweep results as a heatmap over the (t1, tM) threshold grid

    t1 = np.load(f'Outputs/msqe_thresholds_sweep_t1_N_{N}_b_adc_{b_adc}_delta_imc_{delta_imc}.npy')
    tM = np.load(f'Outputs/msqe_thresholds_sweep_tM_N_{N}_b_adc_{b_adc}_delta_imc_{delta_imc}.npy')
    msqe_val = np.load(f'Outputs/msqe_thresholds_msqe_val_N_{N}_b_adc_{b_adc}_delta_imc_{delta_imc}.npy')
    msqe_val_normalized = msqe_val/(delta_imc**2)

    fig, ax = plt.subplots(figsize=(8, 6))

    # Scatter plot
    sc = ax.scatter(t1, tM, c=10 * np.log10(msqe_val_normalized), cmap='viridis', s=20, alpha=0.8, vmin=-30, vmax=30)
    ...
    plt.savefig(f'Figures/msqe_thresholds_sweep_N_{N}_b_acd_{b_adc}_delta_imc_{delta_imc}.png', format='png', bbox_inches='tight')
    plt.show()
...
if __name__ == '__main__':
    ...
    npoints = N*2
    # msqe_thresholds_sweep_gaussian(N, p, b_adc, delta_imc, npoints)
    # msqe_thresholds_sweep_gaussian(N, p, b_adc-1, delta_imc, npoints)
    plot_msqe(N, b_adc, delta_imc)
